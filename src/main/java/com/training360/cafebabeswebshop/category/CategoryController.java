...
public class CategoryController {
    ...
    public Object getCategory(@PathVariable String name){
        ...
        if(categoryValidator.isValidName(name) && names.contains(name)){
            ...
            return new ResultStatus(ResultStatusE.NOT_OK, "Helytelen név");
        }
    }
    ...
    public ResultStatus updateCategory(@PathVariable long id, @RequestBody Category category){
        ...
        if(categoryValidator.isValidOrder(category.getOrdinal())) {
                ...
                return new ResultStatus(ResultStatusE.OK, "Kategória sikeresen módosítva");
        }
        if(!categoryValidator.isValidName(category.getName())){
            return new ResultStatus(ResultStatusE.NOT_OK, "Az adott név már létezik");
        }
        else {
            ...
            return new ResultStatus(ResultStatusE.NOT_OK, "az adott sorszámnak a meglévők között kell lennie!");
        }
    }
}